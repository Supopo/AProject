...
public class Logger extends FrameLayout implements Thread.UncaughtExceptionHandler, Application.ActivityLifecycleCallbacks {
    private static boolean showLogcat = true; //正式环境(false)不打印日志，也不能唤起app的debug界面
    private static boolean checkLeak = false; //是否展示内存泄露信息
    ...
    private Logger(final Context context) {
        ...
        //取value对应SP的像素值
        final float v = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 12, getResources().getDisplayMetrics());
        //日志容器
        ...
        //设置日志窗口宽高
        LayoutParams layoutParams = new LayoutParams(widthPixels * 2 / 3, heightPixels / 3, Gravity.CENTER);
...
//                Toast.makeText(mContext, "发生内存泄漏:" + s, Toast.LENGTH_SHORT).show();
    ...
    private void print(int type, String tag, String msg) {
        if (!showLogcat || me == null || type < mFilterLevel + 2)
            return;
    ...
    private void addText(int type, String text) {
        //设置日志颜色
        String[] level = new String[]{"#FFFFFF", "", "#FFFFFF", "#131417", "#00ff00", "#EFC429", "#FF0000"};
    ...
    //快速点击三下
    private void checkSwitchEx(long dis) {
        if (dis < 500) {
            mShortClick++;
